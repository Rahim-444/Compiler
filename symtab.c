...
int getArraySize(SymbolEntry* entry) {
    if (entry == NULL || !isArray(entry)) return -1;
    return entry->size;
}
...
void checkArrayAccess(char* name, int idx, int exprType, int isConstExpr, int exprValue, int line, int column) {
    ...
    if (!isArray(entry)) {
        sprintf(errorMsg, "Cannot use array indexing on non-array variable '%s'", name);
        semanticError(errorMsg, line, column);
        return;
    }
...
void checkArrayAssignment(char* name, int idx, int exprType, int assignType, int line, int column) {
    ...
    if (entry == NULL || !isArray(entry)) return;
...
void checkConditionType(int exprType, const char* context, int line, int column) {
    char errorMsg[256];
    if (exprType != TYPE_INT) {
        sprintf(errorMsg, "Condition in %s must evaluate to a boolean (Int type)", context);
        semanticError(errorMsg, line, column);
    }
}
...
void checkDivisionByZero(int isConstant, int type, void* value, int line, int column) {
    ...
    int isZero = (type == TYPE_INT) ? (*(int*)value == 0) : (*(float*)value == 0.0f);
    if (isZero) {
